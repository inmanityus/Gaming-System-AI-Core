...
function Stop-ApplicationServersOnly {
    ...
    Write-Host "`n[PROTECT] MCP-SAFE SERVER SHUTDOWN`n" -ForegroundColor Cyan
    
    # Get MCP server PIDs first
    $mcpPids = Get-MCPServerProcesses
    
    if ($mcpPids.Count -gt 0) {
        Write-Host "[OK] Protected $($mcpPids.Count) MCP server(s)`n" -ForegroundColor Green
    } else {
        Write-Host "[WARNING] No MCP servers detected (they may not be running)`n" -ForegroundColor Yellow
    }
    ...
    Write-Host "`n[OK] Killed $killedCount application server(s)" -ForegroundColor Green
    Write-Host "[OK] MCP servers remain protected`n" -ForegroundColor Green
}
...
function Test-MCPProtection {
    ...
    Write-Host "`n[TEST] MCP PROTECTION TEST`n" -ForegroundColor Cyan
    ...
    if ($mcpPids.Count -eq 0) {
        Write-Host "  [WARNING] No MCP servers detected!" -ForegroundColor Yellow
        ...
        $mcpPids | ForEach-Object {
            ...
            if ($process) {
                Write-Host "  [OK] PID: $_" -ForegroundColor Green
            }
        }
        Write-Host ""
    }
