...
if (Test-Path "project-config.md") {
    ...
    Write-Host "   ✓ project-config.md updated" -ForegroundColor Green
}
...
if (Test-Path "$env:USERPROFILE\.cursor\Deployment\Global\.env.txt") {
    ...
    Write-Host "   ⚠ .env template not found, skipping" -ForegroundColor Yellow
}
...
if (Test-Path "scripts\Sync-Global-Rules.ps1") {
    ...
    Write-Host "   ✓ Global repository linked" -ForegroundColor Green
} else {
    Write-Host "   ⚠ Sync-Global-Rules.ps1 not found" -ForegroundColor Yellow
}
...
# Step 6: Create initial commit and push to GitHub
Write-Host "`n[6/7] Creating initial commit and pushing to GitHub..." -ForegroundColor Yellow
git add -A
git commit -m "chore(cursor): initial project setup for $ProjectName

Project Type: $ProjectType
Database: $dbName
Package: $packageName

Generated by automated setup script [chat:project-setup]"

Write-Host "   ✓ Initial commit created" -ForegroundColor Green

# Push to GitHub (creates private repo if needed)
if (Test-Path "Global-Scripts\git-push-to-github.ps1") {
    Write-Host "   Pushing to GitHub..." -ForegroundColor Yellow
    pwsh -ExecutionPolicy Bypass -File "Global-Scripts\git-push-to-github.ps1"
    Write-Host "   ✓ Pushed to GitHub (private repo created if needed)" -ForegroundColor Green
} else {
    Write-Host "   ⚠ GitHub push script not found - run manually later" -ForegroundColor Yellow
    Write-Host "   Run: pwsh -ExecutionPolicy Bypass -File 'Global-Scripts\git-push-to-github.ps1'" -ForegroundColor Gray
}
...
if (Test-Path "startup.ps1") {
    ...
    Write-Host "   ⚠ startup.ps1 not found" -ForegroundColor Yellow
}
